...
APFPathPlanner::APFPathPlanner()
{
	...
	apf_ = new potbot_lib::ArtificialPotentialFieldROS("potential_field");
	apf_planner_ = new potbot_lib::path_planner::APFPathPlannerROS("path_planner", apf_);
	...
	while (ros::ok())
	{
		...
		apf_->setRobot(robot_.pose.pose);
		apf_->setGoal(goal_);
		apf_->clearObstacles();
		...
		double init_yaw = tf2::getYaw(robot_.pose.pose.orientation);
		
		if (isnan(init_yaw)) init_yaw = 0;

		timer.start("path");
		// apf_planner_->createPath(init_yaw);
		apf_planner_->createPathWithWeight(init_yaw);
		apf_planner_->bezier();
